...
Stable error codes: `AUTH_REQUIRED`, `FORBIDDEN`, `VALIDATION_FAILED`, `STATE_CONFLICT`, `IDEMPOTENCY_CONFLICT`, `PAYMENT_PENDING`, `REVIEW_REQUIRED`, `RATE_LIMITED` (429, added by [ADR-007](../12-decisions/adr-007-authentication-sessions-and-rbac.md) for auth throttling), and `INTERNAL` for unmapped 5xx. Never expose stack traces or raw provider errors.
...
Auth uses the hybrid transport of [ADR-006](../12-decisions/adr-006-api-and-scaffolding-conventions.md) §9, implemented per [ADR-007](../12-decisions/adr-007-authentication-sessions-and-rbac.md): `Authorization: Bearer` by default, or httpOnly cookies with double-submit CSRF for first-party web/operations (opt in with `x-auth-transport: cookie`). Sensitive endpoints (login, register, refresh, step-up, change-password) are rate-limited per client and return `RATE_LIMITED` with a `Retry-After` header when exceeded. Server-side authorization is enforced by the domain permission matrix; sensitive actions additionally require step-up authentication.
...
Read CLAUDE.md, 12-decisions/adr-005-stack-and-tooling.md, 12-decisions/adr-006-api-and-scaffolding-conventions.md, 12-decisions/adr-007-authentication-sessions-and-rbac.md, 13-build-blueprint/repository-and-api-contract.md, and 11-quality/acceptance-testing.md.
