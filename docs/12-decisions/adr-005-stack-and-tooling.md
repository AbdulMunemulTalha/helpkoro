...
operation) the _default_, so a solo developer cannot silently skip it.
...
| API framework          | **NestJS on the Fastify adapter**                                          | Resolves the Fastify/NestJS fork. Fastify's speed + Nest guards/interceptors/DI that _enforce_ authz, idempotency, and audit as cross-cutting defaults. Modules map 1:1 to the documented API modules.    |
